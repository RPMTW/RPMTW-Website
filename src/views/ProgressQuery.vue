<template>
    <div id="ProgressQuery">
        ...
        <div class="sets flex">
            <div>
                <input id="search" type="text" placeholder="請輸入CurseForgeID" aria-label="請輸入CurseForgeID"
...
let setRoundProgress = () => {
    for (let i of $(".circle-bar")) {
        ...
        if (!txt.text().includes("NaN")) {
            ...
            if (percent <= 50) {
                $(i).find(".circle-bar-right").css("transform", `rotate(${percent * 10}deg)`);
            } else {
                $(i).find(".circle-bar-right").css({
                    transform: "rotate(0deg)",
                    "background-color": baseColor,
                });
                $(i).find(".circle-bar-left").css("transform", `rotate(${(percent - 50) * 3.6}deg)`);
            }
        } else {
            txt.text("0%");
        }
...
let checkInValue = (these) => {
    ...
    return (_class ? _class.split().includes("chosen") : false) ? these.attr(
        "click-type") : null;
};
export default {
    name: "ProgressQuery",
                    ...
                    if (i) {
                        Object.keys(i).map((val) => {
                            ...
                            if (val.includes(txt) || i[val].ModID.includes(txt)) {
                                $(".output").append(`<div><div><h5>CurseForgeID: ${val}<br>模組ID: ${i[val].ModID}<h5></div><div class="circle-bar"><div class="circle-bar-left"></div><div class="circle-bar-right"></div><div class="mask"><span class="percent">${parseInt(i[val].Progress)}%</span></div></div></div>`);
                            }
                        });
                    }
                }
                ...
                for (let i of searchData) {
                    if (i) {
                        Object.keys(i).map((val) => {
                            if (++l > 10) return 
                            $(".output").append(
                                `<div><div><h5>CurseForgeID: ${val}<br>模組ID: ${i[val].ModID}<h5></div><div class="circle-bar"><div class="circle-bar-left"></div><div class="circle-bar-right"></div><div class="mask"><span class="percent">${parseInt(i[val].Progress)}%</span></div></div></div>`
                            );
                        });
                    }
                }
    ...
    mounted() {
        ["1.12", "1.16", "1.17"].map((item) => {
            $.getJSON(
                `https://raw.githubusercontent.com/RPMTW/RPMTW-website-data/main/data/CrowdinIndex-${item}.json`, (data) => {
